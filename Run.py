...
class main_test(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.read_SN.clicked.connect(self.readSN_PeVoa)
    ...
    # read the SN from EEPROM
    def readSN_PeVoa(self):
        '''
        Add function fo read SN for PeSkew and VOA cal tester
        :return: NA
        '''
        try:
            if self.Test_item.currentText()=='PeSkew':
                cb.open_board_PeSkew(self, self.ctrl_port)
            elif self.Test_item.currentText()=='VOAcal':
                cb.open_board_VOAcal(self, self.ctrl_port)
            s = gf.read_eeprom_PeVoa(self)
            self.print_out_status('已获取SN:{}'.format(s))
            self.lineEdit.setText(s)
            self.CtrlB.close()
        except Exception as e:
    ...
    def Start_test(self):
        ...
        if self.test_type=='Normal':
            if self.test_flag=='DC':
                ...
                # self.channel = [9, 11, 13, 15, 39, 43, 66, 68, 70, 72]
                self.channel = [9, 11, 13, 39, 68, 72]
                self.update_config()
    ...
    def MPC_GBS_Find_Position_0930(self,Func,Maxmize,limit=0.0012):
        '''
        MPC model is GBS-PDL-1
        Function to find the position either to maximize or minimize the output power.
        :param Func:PDX,PDY or DIFF to choose the function to adjust, either X min/max Y min/max or balance
    ...
    # 12341-1234-1234-1234
    def MPC_GBS_Find_Position(self, Func, Maxmize, limit=0.002):
        '''
        MPC model is GBS-PDL-1
        Function to find the position either to maximize or minimize the output power.
        :param Func:PDX,PDY or DIFF to choose the function to adjust, either X min/max Y min/max or balance
        ...
        Inc = 5
        print('Iteration NO.1:')
        setCurW, curX, curY = self.MPC_GBS_Find_XYZ(Func, 'W', C, R, Inc, Maxmize, 0.08)
        time.sleep(0.3)
        pd_currentX += curX
        pd_currentY += curY
        index_mark.append(len(pd_currentX))
        if (Func == 'PDX' and not Maxmize) or (Func == 'PDY' and Maxmize):
            ...
            if min(pd_currentY) < limit: return pd_currentX, pd_currentY, index_mark
        setCurX, curX, curY = self.MPC_GBS_Find_XYZ(Func, 'X', C, R, Inc, Maxmize, 0.08)
        time.sleep(0.3)
        pd_currentX += curX
        pd_currentY += curY
        index_mark.append(len(pd_currentX))
        if (Func == 'PDX' and not Maxmize) or (Func == 'PDY' and Maxmize):
            ...
            if min(pd_currentY) < limit: return pd_currentX, pd_currentY, index_mark
        setCurY, curX, curY = self.MPC_GBS_Find_XYZ(Func, 'Y', C, R, Inc, Maxmize, 0.08)
        time.sleep(0.3)
        pd_currentX += curX
        pd_currentY += curY
        index_mark.append(len(pd_currentX))
        if (Func == 'PDX' and not Maxmize) or (Func == 'PDY' and Maxmize):
            ...
            if min(pd_currentY) < limit: return pd_currentX, pd_currentY, index_mark
        setCurZ, curX, curY = self.MPC_GBS_Find_XYZ(Func, 'Z', C, R, Inc, Maxmize, 0.08)
        time.sleep(0.3)
        pd_currentX += curX
        pd_currentY += curY
        index_mark.append(len(pd_currentX))
        ...
        Inc = 3
        print('Iteration NO.2:')
        setCurW, curX, curY = self.MPC_GBS_Find_XYZ(Func, 'W', setCurW, R, Inc, Maxmize, 0.08)
        time.sleep(0.3)
        pd_currentX += curX
        pd_currentY += curY
        index_mark.append(len(pd_currentX))
        if (Func == 'PDX' and not Maxmize) or (Func == 'PDY' and Maxmize):
            ...
            if min(pd_currentY) < limit: return pd_currentX, pd_currentY, index_mark
        setCurX, curX, curY = self.MPC_GBS_Find_XYZ(Func, 'X', setCurX, R, Inc, Maxmize, 0.08)
        time.sleep(0.3)
        pd_currentX += curX
        pd_currentY += curY
        index_mark.append(len(pd_currentX))
        if (Func == 'PDX' and not Maxmize) or (Func == 'PDY' and Maxmize):
            ...
            if min(pd_currentY) < limit: return pd_currentX, pd_currentY, index_mark
        setCurY, curX, curY = self.MPC_GBS_Find_XYZ(Func, 'Y', setCurY, R, Inc, Maxmize, 0.08)
        time.sleep(0.3)
        pd_currentX += curX
        pd_currentY += curY
        index_mark.append(len(pd_currentX))
        if (Func == 'PDX' and not Maxmize) or (Func == 'PDY' and Maxmize):
            ...
            if min(pd_currentY) < limit: return pd_currentX, pd_currentY, index_mark
        setCurZ, curX, curY = self.MPC_GBS_Find_XYZ(Func, 'Z', setCurZ, R, Inc, Maxmize, 0.08)
        time.sleep(0.3)
        pd_currentX += curX
        pd_currentY += curY
        index_mark.append(len(pd_currentX))
...
class WorkThread(QThread):
    ...
    def run(self):
        #try:
            if mawin.test_flag=="DC":
                ...
                #for i in range(20):
                    #print('+++++++++++++++The {} time repeat test...+++++++++++++++'.format(i+1))
                # self.PEskew_test_ave_new(40)
                self.PEskew_test_ave_new_TIAcheck()
    ...
    def DC_test(self):
        ...
        *****Start the whole test steps*****
        ...
        retest_ch=[]
        test_result,retest_ch=self.er_retest(test_result,config,noipwr,15,20,-3)
        #generate the report and print out the log file
        if len(retest_ch)>0:
            report_name=sn+'_Retest_'+timestamp+'.csv'
            report_file=os.path.join(report_path3,report_name)
        ...
        #copy the local data to network folder
        if not network_path==False:
            shutil.copytree(report_path3,network_path,dirs_exist_ok=True)
        ...
        *****Start the whole test steps*****
        ...
        for i in range(len(mawin.channel)):
            ...
            while sta!='255':
                ...
                #add log record
                time.sleep(0.1)
                mawin.CtrlB.read_all()
                mawin.CtrlB.write(b'print_ber 0 0\n\r')
                print(mawin.CtrlB.read_until(b'shell>\r\n'))
                time.sleep(0.1)
                mawin.CtrlB.read_all()
                mawin.CtrlB.write(b'get_line_ingress_agc_status\n\r')
                print(mawin.CtrlB.read_until(b'AGC digital gain'))
                time.sleep(0.1)
                mawin.CtrlB.read_all()
                mawin.CtrlB.write(b'fpga_ad_get_value 255\n\r')
                print(mawin.CtrlB.read_until(b'ADC_DRV_VDCOUT'))
                time.sleep(0.1)
                mawin.CtrlB.read_all()
    ...
    def PEskew_test_ave(self,ave=10):
        '''
        #Phase error and Skew test with jinli and nianyu namely control FS400 to receive and module to transmit
        :param ave: average times pass in
        :return:NA
        '''
        # firstly rename the file of ICR test config of 'Board up,Drv up,Drv down'
        mawin.board_up = os.path.join(mawin.config_path, 'Setup_brdup_CtrlboardA001_20220113_PEskew.txt')
        ...
        *****Start the whole test steps*****
        ...
        test_result = DataFrame(columns=('SN', 'RX_BW_DESK', 'TEMP', 'DATE', 'TIME', '400G_CH',
                                         ...
                                         'RX_PE_X_min', 'RX_PE_Y_min',
                                         'RX_PE_X','RX_PE_Y', 'RX_X_Skew', 'RX_Y_Skew'))
        ave_mid_data=DataFrame(columns=('CH', 'pex1', 'pex2', 'pex3', 'pex4', 'pex5','pex6', 'pex7', 'pex8', 'pex9', 'pex10',
                                        'pey1', 'pey2', 'pey3', 'pey4', 'pey5', 'pey6', 'pey7', 'pey8', 'pey9', 'pey10'
                                        ))
        mid_file = os.path.join(report_path3, 'X_Y_rawData.csv')
        # create list to store the result
        tt = []
        config = [sn, mawin.desk, mawin.temp[0]] + timestamp.split('_')
        report_name = sn + '_PeSkew_' + timestamp + '.csv'
        ...
        for i in range(len(mawin.channel)):
            ...
            # print('ITLA opened...')
            #***---------Switch JinLi wavelength(Tx)--------***
            if not jl.set_JINLIwave(mawin,ch):
                print('Can not set to the wavelength intended please check!')
                return
            print('JINLI wavelength switched to ch{}...'.format(ch))
            #***---------Judge system connectivity--------***
            sta='0'
            ...
            while sta!='255':
                ...
                count += 1
            self.sig_print.emit('系统连通，开始获取Phase error和Skew...')
            #average repeat the test for given times
            pe_x_ave_list=[]
            pe_y_ave_list=[]
            skew_x_ave_list=[]
            skew_y_ave_list=[]
            ...
            while count_ave<ave:
                ...
                resu=mawin.CtrlB.read_until(b'rx_csr_max_v').decode('utf-8')
                count_ave += 1
                print('****************---The {} time repeat test---******************'.format(count_ave))
                print(resu)
                result_tmp=resu.split('\n')
                flag_PEx='rx_angle_average_h'
                flag_PEy = 'rx_angle_average_v'
                ...
                if len(result_peskew_ave)!=4 or len(result_peskew_max)!=4 or len(result_peskew_min)!=4:
                    print('Average or Max or Min返回值数量为{} {} {},不等于4个，请检查！'.format(len(result_peskew_ave),len(result_peskew_max),len(result_peskew_min)))
                    return
                pe_x_ave   = float(re.split('[: ]', result_peskew_ave[0])[1])
                pe_y_ave   = float(re.split('[: ]', result_peskew_ave[1])[1])
                skew_x_ave = float(re.split('[: ]', result_peskew_ave[2])[1])
                skew_y_ave = float(re.split('[: ]', result_peskew_ave[3])[1])

                pe_x_max   = float(re.split('[: ]', result_peskew_max[0])[1])
                pe_y_max   = float(re.split('[: ]', result_peskew_max[1])[1])
                skew_x_max = float(re.split('[: ]', result_peskew_max[2])[1])
                skew_y_max = float(re.split('[: ]', result_peskew_max[3])[1])

                pe_x_min    =float(re.split('[: ]', result_peskew_min[0])[1])
                pe_y_min    =float(re.split('[: ]', result_peskew_min[1])[1])
                skew_x_min  =float(re.split('[: ]', result_peskew_min[2])[1])
                skew_y_min  =float(re.split('[: ]', result_peskew_min[3])[1])

                pe_x_ave_list.append(pe_x_ave)
                pe_y_ave_list.append(pe_y_ave)
                skew_x_ave_list.append(skew_x_ave)
                skew_y_ave_list.append(skew_y_ave)
                pe_x_max_list.append(pe_x_max)
                pe_y_max_list.append(pe_y_max)
                skew_x_max_list.append(skew_x_max)
                skew_y_max_list.append(skew_y_max)
                pe_x_min_list.append(pe_x_min)
                pe_y_min_list.append(pe_y_min)
                skew_x_min_list.append(skew_x_min)
                skew_y_min_list.append(skew_y_min)

                if abs(pe_x_max) >= abs(pe_x_min):
                    pe_x = pe_x_max
                else:
                    pe_x = pe_x_min
                if abs(pe_y_max) >= abs(pe_y_min):
                    pe_y = pe_y_max
                else:
                    pe_y = pe_y_min

                pe_x_list.append(pe_x)
                pe_y_list.append(pe_y)

            pe_x_ave=np.average(pe_x_ave_list)
            pe_y_ave=np.average(pe_y_ave_list)
            skew_x_ave=np.average(skew_x_ave_list)
            skew_y_ave=np.average(skew_y_ave_list)
            pe_x_max=np.average(pe_x_max_list)
            pe_y_max=np.average(pe_y_max_list)
            skew_x_max=np.average(skew_x_max_list)
            skew_y_max=np.average(skew_y_max_list)
            pe_x_min=np.average(pe_x_min_list)
            pe_y_min=np.average(pe_y_min_list)
            skew_x_min=np.average(skew_x_min_list)
            skew_y_min=np.average(skew_y_min_list)
            ...
            # else:
            #     pe_y = pe_y_min
            # pe_x = np.average(pe_x_list)
            # pe_y = np.average(pe_y_list)
            pe_x = np.median(pe_x_list)
            pe_y = np.median(pe_y_list)
            skew_x = skew_x_max
            skew_y = skew_y_max
            #save mid data
            row_add=[ch]+pe_x_list+pe_y_list
            ave_mid_data.loc[p-1]=row_add
            ave_mid_data.to_csv(mid_file, index=False)

            print('ch,pe_x_ave, pe_y_ave,pe_x_max, pe_y_max,pe_x_min, pe_y_min,pe_x, pe_y, skew_x, skew_y:\n',
    ...
    def PEskew_test_ave_new(self,ave=40):
        '''
        #Phase error and Skew test with jinli and nianyu namely control FS400 to receive and module to transmit
        :param ave: average times pass in
        :return:NA
        ...
        # mawin.CtrlB.write(b'show_itla 0\n\r');
        # time.sleep(0.5)
        # re_itla = mawin.CtrlB.read_until(b'run_state').decode('utf-8', errors='ignore').split('\n')
        # print(re_itla)
        # try:
        #     re_itla1 = [i.split(' ')[-1] for i in re_itla if 'g_itla_model' in i][0]
        # except Exception as e:
        ...
        #mawin.channel = [i for i in range(9, 73)]
        #mawin.channel = [i for i in range(9, 12)]
        #mawin.channel = [i for i in range(9,73,3)]
        #mawin.channel = [9, 11, 13, 15, 17, 35, 37, 39, 41, 43, 64, 66, 68, 70, 72]
        #mawin.update_config()
        ...
        *****Start the whole test steps*****
        ...
        for i in range(1,int(ave)+1):
            column_pe.append('pex_MaxAbs{}'.format(i))
        for i in range(1,int(ave)+1):
            column_pe.append('pey_MaxAbs{}'.format(i))

        #add ave,min,max raw data record
        for i in range(1,int(ave)+1):
            column_pe.append('pex_ave{}'.format(i))
        for i in range(1,int(ave)+1):
            column_pe.append('pey_ave{}'.format(i))
        for i in range(1,int(ave)+1):
            column_pe.append('pex_min{}'.format(i))
        for i in range(1,int(ave)+1):
            column_pe.append('pey_min{}'.format(i))
        for i in range(1,int(ave)+1):
            column_pe.append('pex_max{}'.format(i))
        for i in range(1,int(ave)+1):
            column_pe.append('pey_max{}'.format(i))

        column_raw=['CH']+column_pe


        # ave_mid_data=DataFrame(columns=('CH', 'pex1', 'pex2', 'pex3', 'pex4', 'pex5','pex6', 'pex7', 'pex8', 'pex9', 'pex10',
        #                                 'pey1', 'pey2', 'pey3', 'pey4', 'pey5', 'pey6', 'pey7', 'pey8', 'pey9', 'pey10'
        #                                 ))
        ave_mid_data = DataFrame(columns=column_raw)
        mid_file = os.path.join(report_path3, 'X_Y_rawData.csv')
        # create list to store the result
        tt = []
        config = [sn, mawin.desk, mawin.temp[0]] + timestamp.split('_')
        ...
        # #Open Jinli light
        # mawin.JinLi.write(b'msa_write 0xb010 0\r\n')
        # print(mawin.JinLi.read_until(b'MSA_W 0xB010 0x0000 OK'))
        # print('Jinli light opened...')
        ...
        for i in range(len(mawin.channel)):
            ...
            mawin.CtrlB.read_all()
            mawin.CtrlB.write(b'itla_read 0 0x30\n\r')
            re_wl=mawin.CtrlB.read_until(b'shell').decode('utf-8', errors='ignore')
            print(re_wl)
            while not 'read_itla' in re_wl:
                time.sleep(1)
                mawin.CtrlB.write(b'itla_read 0 0x30\n\r')
                re_wl = mawin.CtrlB.read_until(b'shell').decode('utf-8', errors='ignore')
                print(re_wl)
            fre_get=[int(i,16) for i in re_wl.split('\r\n') if '0x' in i and not 'itla' in i][0]
            #fre_get=int(re_wl.split('\n')[2].strip().replace('\r',''),16)
            cou1=0
            while not fre_get == ch_toset:
                print('The {} time set wavelength...'.format(cou1+1))
                ...
                #     time.sleep(1)
                #     mawin.CtrlB.write(b'itla_read 0 0x30\n\r')
                #     re_wl = mawin.read_until(b'shell').decode('utf-8', errors='ignore')
                #     print(re_wl)
                # fre_get = int(re_wl.split('\n')[2].strip().replace('\r', ''), 16)
                cou1+=1
                if cou1>3:
            ...
            while sta!='255':
                ...
                mawin.CtrlB.write(b'show_dsp_ber_info\n\r')#;time.sleep(0.5)
                #ret=mawin.CtrlB.read_all().decode('utf-8')
                ret=mawin.CtrlB.read_until(b'line_fas_ber').decode('utf-8', errors='ignore')
                print('DSP information:\n',ret)
                #tmp1=re.split('[:,\r]',tmp[ind+1]+tmp[ind+2])
                ret_1=ret.split('\n')
                sta_list=[i[i.index('(')+1:i.index(')')] for i in ret_1 if 'dsp_state' in i]
            ...
            mawin.CtrlB.write(b'trigger_monitors 0\n\r')
            time.sleep(2)
            print(mawin.CtrlB.read_all().decode('utf-8'))
            mawin.CtrlB.write(b'dsp_control_task 26 1 1\n\r')
            time.sleep(2)
            ...
            print(mawin.CtrlB.read_all().decode('utf-8'))
            mawin.CtrlB.write(b'trigger_monitors 1\n\r')
            print(mawin.CtrlB.read_until(b'00  00  04  00  00  00'))
            print(mawin.CtrlB.read_all().decode('utf-8'))
            ...
            pe_x_ave = np.median(pe_x_ave_list)
            pe_y_ave = np.median(pe_y_ave_list)
            skew_x_ave = np.median(skew_x_ave_list)
            skew_y_ave = np.median(skew_y_ave_list)
            pe_x_max = np.median(pe_x_max_list)
            pe_y_max = np.median(pe_y_max_list)
            skew_x_max = np.median(skew_x_max_list)
            skew_y_max = np.median(skew_y_max_list)
            pe_x_min = np.median(pe_x_min_list)
            pe_y_min = np.median(pe_y_min_list)
            skew_x_min = np.median(skew_x_min_list)
            skew_y_min = np.median(skew_y_min_list)
            ...
            # pe_x =-pe_x_ave
            # pe_y =pe_y_ave-3
            #2022-11-07 11：36：00 直接取中位数，不做数据处理
            pe_x = pe_x_ave
            pe_y = pe_y_ave
            # skew_x = skew_x_max
            # skew_y = skew_y_max
            ...
            skew_y=skew_y_ave
            #save mid data
            row_add=[ch]+pe_x_list+pe_y_list+pe_x_ave_list+pe_y_ave_list+pe_x_min_list+pe_y_min_list+pe_x_max_list+pe_y_max_list
            ave_mid_data.loc[p-1]=row_add
            ave_mid_data.to_csv(mid_file, index=False)
        ...
        print('LO laser closed...')
        # # Close Jinli light
        # mawin.JinLi.write(b'msa_write 0xb010 8000\r\n')
        # print(mawin.JinLi.read_until(b'MSA_W 0xB010 0x1F40 OK'))
        # print('Jinli light closed...')
    ...
    def PEskew_test_ave_new_TIAcheck(self, ave=40):
        '''
        add TIA related specs-20221124
        2022-11-10 add TIA RF crosstalk,TIA MGC/AGC, Jinli laser off
        #Phase error and Skew test with jinli and nianyu namely control FS400 to receive and module to transmit
        :param ave: average times pass in
        :return:NA
        '''
        ...
        # open Jinli light first before connectivity test
        mawin.JinLi.write(b'ittra_write 0xb013 0x0\r\n')
        print(mawin.JinLi.read_until(b'ittra mdio write OK'))
        time.sleep(0.2)
        mawin.JinLi.read_all()
        # read status
        mawin.JinLi.write(b'ittra_read 0xb013 0\r\n')
        ret_tia = mawin.JinLi.read_until(b'ittra mdio read OK').decode('utf-8')  # .split('\r\n')
        cnt = 0
        while not '0xb013 0x0000' in ret_tia:
            mawin.JinLi.write(b'ittra_write 0xb013 0x0\r\n')
            print(mawin.JinLi.read_until(b'ittra mdio write OK'))
            time.sleep(0.2)
            mawin.JinLi.read_all()
            mawin.JinLi.write(b'ittra_read 0xb013 0\r\n')
            ret_tia = mawin.JinLi.read_until(b'ittra mdio read OK').decode('utf-8')
            cnt += 1
            if cnt > 2:
                print('Jinli Laser open failed!')
                return
        ...
        *****Start the whole test steps*****
        ...
        for i in range(len(mawin.channel)):
            ...
            if not jl.set_JINLIwave(mawin, ch):
                ...
                return
            print('JINLI wavelength switched to ch{}...'.format(ch))
            print('\nJINLI切换完成：', gf.get_timestamp())

            # ********************TIA test only one wavelength, choose the first one*****************
            column_raw = ['RF_on_ITLA_on_XQ', 'RF_on_ITLA_on_XI', 'RF_on_ITLA_on_YQ', 'RF_on_ITLA_on_YI',
                          'RF_off_ITLA_on_XQ', 'RF_off_ITLA_on_XI', 'RF_off_ITLA_on_YQ', 'RF_off_ITLA_on_YI',
                          'RF_off_ITLA_off_XQ', 'RF_off_ITLA_off_XI', 'RF_off_ITLA_off_YQ', 'RF_off_ITLA_off_YI',
                          'RF_crosstalk_XQ', 'RF_crosstalk_XI', 'RF_crosstalk_YQ', 'RF_crosstalk_YI',
                          'TIA_MGC_DSPin_XQ', 'TIA_MGC_DSPin_XI', 'TIA_MGC_DSPin_YQ', 'TIA_MGC_DSPin_YI',
                          'TIA_MGC_PKD_XQ', 'TIA_MGC_PKD_XI', 'TIA_MGC_PKD_YQ', 'TIA_MGC_PKD_YI',
                          'TIA_AGC_PKD_XQ', 'TIA_AGC_PKD_XI', 'TIA_AGC_PKD_YQ', 'TIA_AGC_PKD_YI',
                          'TIA_AGC_GC_YI', 'TIA_AGC_GC_YQ', 'TIA_AGC_GC_XI', 'TIA_AGC_GC_XQ',
                          'TIA_AGC_DSPin_XQ', 'TIA_AGC_DSPin_XI', 'TIA_AGC_DSPin_YQ', 'TIA_AGC_DSPin_YI','SN'
                          ]
            TIA_result = DataFrame(columns=column_raw)
            TIA_file = os.path.join(report_path3, '{}_TIA_RF_test_result_{}.csv'.format(sn,timestamp))
            # open Jinli finally.
            TIA_AGC_PKD_XQ, TIA_AGC_PKD_XI, TIA_AGC_PKD_YQ, TIA_AGC_PKD_YI=[None]*4
            TIA_AGC_GC_YI, TIA_AGC_GC_YQ, TIA_AGC_GC_XI, TIA_AGC_GC_XQ=[None]*4
            TIA_AGC_DSPin_XQ, TIA_AGC_DSPin_XI, TIA_AGC_DSPin_YQ, TIA_AGC_DSPin_YI=[None]*4
            if p == 1:
                mawin.CtrlB.timeout=10
                ave_tia = 3
                # *************close Jinli Laser*************#
                mawin.JinLi.write(b'ittra_write 0xb013 0x1\r\n')
                print(mawin.JinLi.read_until(b'ittra mdio write OK'))
                time.sleep(0.2)
                mawin.JinLi.read_all()
                # read
                ...
                ret_tia = mawin.JinLi.read_until(b'ittra mdio read OK').decode('utf-8')  # .split('\r\n')
                cnt = 0
                while not '0xb013 0x0001' in ret_tia:
                    mawin.JinLi.write(b'ittra_write 0xb013 0x1\r\n')
                    print(mawin.JinLi.read_until(b'ittra mdio write OK'))
                    time.sleep(0.2)
                    mawin.JinLi.read_all()
                    ...
                    cnt += 1
                    if cnt > 2:
                        print('Jinli Laser close failed!')
                        return

                # read RF power
                RF_on_ITLA_on_XQ, RF_on_ITLA_on_XI, RF_on_ITLA_on_YQ, RF_on_ITLA_on_YI = [None] * 4
                DSP_XQ=0.0
                DSP_XI=0.0
                DSP_YQ=0.0
                DSP_YI=0.0
                for cnt in range(ave_tia):
                    mawin.CtrlB.read_all()
                    mawin.CtrlB.write(b'get_line_ingress_agc_status\r\n')
                    ret = mawin.CtrlB.read_until(b'AGC digital gain').decode('utf-8')
                    time.sleep(0.2)
                    mawin.CtrlB.read_all()
                    tt = re.split('[\t\r\n ]', ret)
                    DSP_XQ_tmp, DSP_XI_tmp, DSP_YQ_tmp, DSP_YI_tmp = [float(i.split(':')[1]) for i in tt if
                                                      'HI' in i or 'HQ' in i or 'VI' in i or 'VQ' in i]
                    DSP_XQ += DSP_XQ_tmp
                    DSP_XI += DSP_XI_tmp
                    DSP_YQ += DSP_YQ_tmp
                    DSP_YI += DSP_YI_tmp
                RF_on_ITLA_on_XQ, RF_on_ITLA_on_XI, RF_on_ITLA_on_YQ, RF_on_ITLA_on_YI = [DSP_XQ / ave_tia, DSP_XI / ave_tia,
                                                                                          DSP_YQ / ave_tia, DSP_YI / ave_tia]
                print('RF_on_ITLA_on_XQ, RF_on_ITLA_on_XI, RF_on_ITLA_on_YQ, RF_on_ITLA_on_YI:\n',
                      [RF_on_ITLA_on_XQ, RF_on_ITLA_on_XI, RF_on_ITLA_on_YQ, RF_on_ITLA_on_YI])

                # switch to RF off status for four channels
                for ch_id in range(4):
                    cmd_set = 'dsp_set_line_mute {} 1 1\r\n'.format(ch_id).encode('utf-8')
                    cmd_read = 'dsp_get_line_mute {}\r\n'.format(ch_id).encode('utf-8')
                    mawin.CtrlB.write(cmd_set)
                    print(mawin.CtrlB.read_until(b'SUCCESS'))
                    time.sleep(0.1)
                    mawin.CtrlB.read_all()
                    mawin.CtrlB.write(cmd_read)
                    ret = mawin.CtrlB.read_until(b'shell>')
                    print(ret)
                    mawin.CtrlB.read_all()
                    time.sleep(0.1)
                    cnt = 0
                    while not b'mute:1' in ret:
                        print('RF not off')
                        mawin.CtrlB.write(cmd_read)
                        ret = mawin.CtrlB.read_until(b'shell>')
                        print(ret)
                        ...
                        cnt += 1
                        if cnt == 3:
                            print('3 times not get RF off, please check')
                            return
                    print('CH{} RF output closed!'.format(ch_id))

                # now get the RF output again
                RF_off_ITLA_on_XQ, RF_off_ITLA_on_XI, RF_off_ITLA_on_YQ, RF_off_ITLA_on_YI = [None] * 4
                DSP_XQ = 0.0
                DSP_XI = 0.0
                DSP_YQ = 0.0
                DSP_YI = 0.0
                for cnt in range(ave_tia):
                    ...
                    DSP_YQ += DSP_YQ_tmp
                    DSP_YI += DSP_YI_tmp
                RF_off_ITLA_on_XQ, RF_off_ITLA_on_XI, RF_off_ITLA_on_YQ, RF_off_ITLA_on_YI = [DSP_XQ / ave_tia,
                                                                                              DSP_XI / ave_tia,
                                                                                              DSP_YQ / ave_tia,
                                                                                              DSP_YI / ave_tia]
                print('RF_off_ITLA_on_XQ, RF_off_ITLA_on_XI, RF_off_ITLA_on_YQ, RF_off_ITLA_on_YI:\n',
                      [RF_off_ITLA_on_XQ, RF_off_ITLA_on_XI, RF_off_ITLA_on_YQ, RF_off_ITLA_on_YI])

                # Close FS400 ITLA
                mawin.CtrlB.write(cmClose)
                print(mawin.CtrlB.read_until(b'ok'))
                time.sleep(0.5)
                print('FS400 LO laser closed...')

                # now get the RF output again and again
                RF_off_ITLA_off_XQ, RF_off_ITLA_off_XI, RF_off_ITLA_off_YQ, RF_off_ITLA_off_YI = [None] * 4
                DSP_XQ = 0.0
                DSP_XI = 0.0
                DSP_YQ = 0.0
                DSP_YI = 0.0
                for cnt in range(ave_tia):
                    ...
                    DSP_YQ += DSP_YQ_tmp
                    DSP_YI += DSP_YI_tmp
                RF_off_ITLA_off_XQ, RF_off_ITLA_off_XI, RF_off_ITLA_off_YQ, RF_off_ITLA_off_YI = [DSP_XQ / ave_tia,
                                                                                                  DSP_XI / ave_tia,
                                                                                                  DSP_YQ / ave_tia,
                                                                                                  DSP_YI / ave_tia]
                print('RF_off_ITLA_off_XQ, RF_off_ITLA_off_XI, RF_off_ITLA_off_YQ, RF_off_ITLA_off_YI:\n',
                      [RF_off_ITLA_off_XQ, RF_off_ITLA_off_XI, RF_off_ITLA_off_YQ, RF_off_ITLA_off_YI])

                # switch to RF on status for four channels
                for ch_id in range(4):
                    cmd_set = 'dsp_set_line_mute {} 0 1\r\n'.format(ch_id).encode('utf-8')
                    cmd_read = 'dsp_get_line_mute {}\r\n'.format(ch_id).encode('utf-8')
                    mawin.CtrlB.write(cmd_set)
                    print(mawin.CtrlB.read_until(b'SUCCESS'))
                    time.sleep(0.1)
                    ...
                    while not b'mute:0' in ret:
                        ...
                        cnt += 1
                        if cnt == 3:
                            print('3 times not get RF on, please check')
                            return
                    print('CH{} RF output opened!'.format(ch_id))
                # Crosstalk calculation
                RF_crosstalk_XQ = RF_on_ITLA_on_XQ - RF_off_ITLA_on_XQ
                RF_crosstalk_XI = RF_on_ITLA_on_XI - RF_off_ITLA_on_XI
                RF_crosstalk_YQ = RF_on_ITLA_on_YQ - RF_off_ITLA_on_YQ
                RF_crosstalk_YI = RF_on_ITLA_on_YI - RF_off_ITLA_on_YI
                print('RF_crosstalk_XQ, RF_crosstalk_XI, RF_crosstalk_YQ, RF_crosstalk_YI:\n',
                      [RF_crosstalk_XQ, RF_crosstalk_XI, RF_crosstalk_YQ, RF_crosstalk_YI])

                # *************************TIA MGC Amplitude**************************#
                # open Jinli light first
                mawin.JinLi.write(b'ittra_write 0xb013 0x0\r\n')
                print(mawin.JinLi.read_until(b'ittra mdio write OK'))
                time.sleep(0.2)
                mawin.JinLi.read_all()
                ...
                while not '0xb013 0x0000' in ret_tia:
                    ...
                    if cnt > 2:
                        print('Jinli Laser open failed!')
                        return
                # open FS400 ITLA output
                mawin.CtrlB.write(cmOpen)
                print(mawin.CtrlB.read_until(b'itla_0_write'))
                time.sleep(0.5)
                print('FS400 ITLA opened...')

                # set FS400 task status
                mawin.CtrlB.write(b'task_ctrl 6 0\r\n')
                ret = mawin.CtrlB.read_until(b'1-run').decode('utf-8')
                print(ret)
                time.sleep(0.2)
                mawin.CtrlB.read_all()
                cnt = 0
                while not 'ctrl:0' in ret:
                    mawin.CtrlB.write(b'task_ctrl 6 0\r\n')
                    ret = mawin.CtrlB.read_until(b'1-run').decode('utf-8')
                    print(ret)
                    time.sleep(0.2)
                    ...
                    cnt += 1
                    if cnt > 2:
                        print('FS400 task status set failed!')
                        return

                # set TIA status
                mawin.CtrlB.write(b'fs400_tia_write 0x0007 0x0002 1\r\n')
                ret = mawin.CtrlB.read_until(b'data[0x0002]').decode('utf-8')
                time.sleep(0.1)
                print(ret)
                mawin.CtrlB.read_all()
                while not 'data[0x0002]' in ret:
                    mawin.CtrlB.write(b'fs400_tia_write 0x0007 0x0002 1\r\n')
                    ret = mawin.CtrlB.read_until(b'data[0x0002]').decode('utf-8')
                    time.sleep(0.1)
                    print(ret)
                    ...
                    cnt += 1
                    if cnt > 2:
                        print('FS400 TIA status set failed!')
                        return
                # config each channel
                for ch_id in ['0015', '0095', '0115', '0195']:
                    #2022-11-12: fs400_tia_write 0x{} 0x0070 1 改为0x0040 减小风险 改成0063 with 5dB固衰
                    cmd_set = 'fs400_tia_write 0x{} 0x0063 1\r\n'.format(ch_id).encode('utf-8')
                    mawin.CtrlB.write(cmd_set)
                    ret = mawin.CtrlB.read_until(b'success').decode('utf-8')
                    time.sleep(0.1)
                    print(ret)
                    cnt = 0
                    while not 'write[0x{}]'.format(ch_id) in ret:
                        mawin.CtrlB.write(cmd_set)
                        ret = mawin.CtrlB.read_until(b'success').decode('utf-8')
                        time.sleep(0.1)
                        mawin.CtrlB.read_all()
                        cnt += 1
                        if cnt == 3:
                            print('3 times not get TIA set, please check')
                            return
                    print('Address {} TIA set!'.format(ch_id))

                # now get the MGC mode RF output
                TIA_MGC_DSPin_XQ, TIA_MGC_DSPin_XI, TIA_MGC_DSPin_YQ, TIA_MGC_DSPin_YI = [None] * 4
                DSP_XQ = 0.0
                DSP_XI = 0.0
                DSP_YQ = 0.0
                DSP_YI = 0.0
                for cnt in range(ave_tia):
                    ...
                    DSP_YQ += DSP_YQ_tmp
                    DSP_YI += DSP_YI_tmp
                TIA_MGC_DSPin_XQ, TIA_MGC_DSPin_XI, TIA_MGC_DSPin_YQ, TIA_MGC_DSPin_YI = [DSP_XQ / ave_tia,
                                                                                          DSP_XI / ave_tia,
                                                                                          DSP_YQ / ave_tia,
                                                                                          DSP_YI / ave_tia]
                print('TIA_MGC_DSPin_XQ, TIA_MGC_DSPin_XI, TIA_MGC_DSPin_YQ, TIA_MGC_DSPin_YI:\n',
                      [TIA_MGC_DSPin_XQ, TIA_MGC_DSPin_XI, TIA_MGC_DSPin_YQ, TIA_MGC_DSPin_YI])
                # get PKD value
                mawin.CtrlB.write(b'fpga_ad_get_value 255\r\n')
                ret = mawin.CtrlB.read_until(b'ADC_DRV_VDCOUT').decode('utf-8')
                time.sleep(0.1)
                print(ret)
                ret_1 = ret.split('\n')
                TIA_MGC_PKD_XQ = int([i[i.index('=') + 2:i.index('mV')] for i in ret_1 if 'ADC_TIA_PKD_XQ' in i][0])
                TIA_MGC_PKD_XI = int([i[i.index('=') + 2:i.index('mV')] for i in ret_1 if 'ADC_TIA_PKD_XI' in i][0])
                TIA_MGC_PKD_YQ = int([i[i.index('=') + 2:i.index('mV')] for i in ret_1 if 'ADC_TIA_PKD_YQ' in i][0])
                TIA_MGC_PKD_YI = int([i[i.index('=') + 2:i.index('mV')] for i in ret_1 if 'ADC_TIA_PKD_YI' in i][0])
                print('TIA_MGC_PKD_XQ,TIA_MGC_PKD_XI,TIA_MGC_PKD_YQ,TIA_MGC_PKD_YI:\n',
                      [TIA_MGC_PKD_XQ, TIA_MGC_PKD_XI, TIA_MGC_PKD_YQ, TIA_MGC_PKD_YI])

                # ****************************TIA AGC PKD and GC**************************#
                # set FS400 task status open
                mawin.CtrlB.write(b'task_ctrl 6 1\r\n')
                ret = mawin.CtrlB.read_until(b'1-run').decode('utf-8')
                print(ret)
                time.sleep(0.2)
                mawin.CtrlB.read_all()
                cnt = 0
                while not 'ctrl:1' in ret:
                    mawin.CtrlB.write(b'task_ctrl 6 1\r\n')
                    ret = mawin.CtrlB.read_until(b'1-run').decode('utf-8')
                    print(ret)
                    time.sleep(0.2)
                    ...
                    if cnt > 2:
                        ...
                        return
                # set TIA status
                mawin.CtrlB.write(b'fs400_tia_write 0x0007 0x0003 1\r\n')
                ret = mawin.CtrlB.read_until(b'success').decode('utf-8')
                time.sleep(0.1)
                print(ret)
                mawin.CtrlB.read_all()
                while not 'data[0x0003]' in ret:
                    mawin.CtrlB.write(b'fs400_tia_write 0x0007 0x0003 1\r\n')
                    ret = mawin.CtrlB.read_until(b'success').decode('utf-8')
                    time.sleep(0.1)
                    print(ret)
                ...
                ret_1 = ret.split('\n')
                TIA_AGC_PKD_XQ = int(
                    [i[i.index('=') + 2:i.index('mV')] for i in ret_1 if 'ADC_TIA_PKD_XQ' in i][0])
                TIA_AGC_PKD_XI = int(
                    [i[i.index('=') + 2:i.index('mV')] for i in ret_1 if 'ADC_TIA_PKD_XI' in i][0])
                TIA_AGC_PKD_YQ = int(
                    [i[i.index('=') + 2:i.index('mV')] for i in ret_1 if 'ADC_TIA_PKD_YQ' in i][0])
                TIA_AGC_PKD_YI = int(
                    [i[i.index('=') + 2:i.index('mV')] for i in ret_1 if 'ADC_TIA_PKD_YI' in i][0])
                print('TIA_AGC_PKD_XQ, TIA_AGC_PKD_XI, TIA_AGC_PKD_YQ, TIA_AGC_PKD_YI:\n',
                      [TIA_AGC_PKD_XQ, TIA_AGC_PKD_XI, TIA_AGC_PKD_YQ, TIA_AGC_PKD_YI])
                # config each channel
                TIA_GC_list = []
                for ch_id in ['0018', '0098', '0118', '0198']:
                    cmd_set = 'fs400_tia_read 0x{}\r\n'.format(ch_id).encode('utf-8')
                    mawin.CtrlB.write(cmd_set)
                    ret = mawin.CtrlB.read_until(b'success').decode('utf-8')
                    print(ret)
                    time.sleep(0.1)
                    mawin.CtrlB.read_all()
                    cnt = 0
                    while not 'read[0x{}]'.format(ch_id) in ret:
                        mawin.CtrlB.write(cmd_set)
                        ret = mawin.CtrlB.read_until(b'success').decode('utf-8')
                        time.sleep(0.1)
                        print(ret)
                        ...
                        cnt += 1
                        if cnt == 3:
                            print('3 times not get TIA read correct value, please check')
                            return
                    GC_Value = ret[ret.index('data') + 5:ret.index('] success')]
                    print(GC_Value)
                    count=0
                    while GC_Value=='0x0000':
                        mawin.CtrlB.write(cmd_set)
                        ret = mawin.CtrlB.read_until(b'success').decode('utf-8')
                        print(ret)
                        time.sleep(0.1)
                        ...
                        while not 'read[0x{}]'.format(ch_id) in ret:
                            ...
                            count+=1
                            if count>2:
                                print('3 times get TIA read 0x0000, please check!')
                                return

                    GC_Value = int(GC_Value, 16)
                    TIA_GC_list.append(GC_Value)
                TIA_AGC_GC_YI, TIA_AGC_GC_YQ, TIA_AGC_GC_XI, TIA_AGC_GC_XQ = TIA_GC_list
                print('TIA_AGC_GC_YI,TIA_AGC_GC_YQ,TIA_AGC_GC_XI,TIA_AGC_GC_XQ:\n',
                      [TIA_AGC_GC_YI, TIA_AGC_GC_YQ, TIA_AGC_GC_XI, TIA_AGC_GC_XQ])

                # now get the AGC mode RF output
                TIA_AGC_DSPin_XQ, TIA_AGC_DSPin_XI, TIA_AGC_DSPin_YQ, TIA_AGC_DSPin_YI = [None] * 4
                DSP_XQ = 0.0
                DSP_XI = 0.0
                DSP_YQ = 0.0
                ...
                for cnt in range(ave_tia):
                    ...
                    DSP_YQ += DSP_YQ_tmp
                    DSP_YI += DSP_YI_tmp
                TIA_AGC_DSPin_XQ, TIA_AGC_DSPin_XI, TIA_AGC_DSPin_YQ, TIA_AGC_DSPin_YI = [DSP_XQ / ave_tia,
                                                                                          DSP_XI / ave_tia,
                                                                                          DSP_YQ / ave_tia,
                                                                                          DSP_YI / ave_tia]
                print('TIA_AGC_DSPin_XQ, TIA_AGC_DSPin_XI, TIA_AGC_DSPin_YQ, TIA_AGC_DSPin_YI:\n',
                      [TIA_AGC_DSPin_XQ, TIA_AGC_DSPin_XI, TIA_AGC_DSPin_YQ, TIA_AGC_DSPin_YI])

                tia_result_raw = [RF_on_ITLA_on_XQ, RF_on_ITLA_on_XI, RF_on_ITLA_on_YQ, RF_on_ITLA_on_YI,
                                  RF_off_ITLA_on_XQ, RF_off_ITLA_on_XI, RF_off_ITLA_on_YQ, RF_off_ITLA_on_YI,
                                  RF_off_ITLA_off_XQ, RF_off_ITLA_off_XI, RF_off_ITLA_off_YQ, RF_off_ITLA_off_YI,
                                  RF_crosstalk_XQ, RF_crosstalk_XI, RF_crosstalk_YQ, RF_crosstalk_YI,
                                  TIA_MGC_DSPin_XQ, TIA_MGC_DSPin_XI, TIA_MGC_DSPin_YQ, TIA_MGC_DSPin_YI,
                ...
                TIA_result.loc[0] = tia_result_raw
                TIA_result_temp=TIA_result.transpose()
                TIA_result_temp.columns=['Result']
                TIA_result_temp.to_csv(TIA_file, index=True)
            ...
            while sta != '255':
                ...
                print('\nline_fas_ber:\n',end1-start)
                mawin.CtrlB.write(b'print_ber 0 0\n\r')
                print(mawin.CtrlB.read_until(b'Framer_D'))
                time.sleep(0.1)
                print(mawin.CtrlB.read_all())
                end2 = time.time()                    #####
                print('\nprint_ber:\n', end2 - end1)  #####
            ...
            while count_ave < ave:
                ...
                flag_SKEWx = 'rx_skew_average_h'
                flag_SKEWy = 'rx_skew_average_v'
                result_peskew_ave = [i for i in result_tmp if
                                     flag_PEx in i or flag_PEy in i or flag_SKEWx in i or flag_SKEWy in i]
                print('Average:\n', result_peskew_ave)
                flag_PEx = 'rx_angle_max_h'
                flag_PEy = 'rx_angle_max_v'
                ...
                print('Min:\n', result_peskew_min)
                if len(result_peskew_ave) != 4 or len(result_peskew_max) != 4 or len(result_peskew_min) != 4:
                    print('Average or Max or Min返回值数量为{} {} {},不等于4个，请检查！'.format(len(result_peskew_ave),
                                                                                               len(result_peskew_max),
                                                                                               len(result_peskew_min)))
                    return
                pe_x_ave = float(re.split('[: ]', result_peskew_ave[0])[1])
                pe_y_ave = float(re.split('[: ]', result_peskew_ave[1])[1])
                skew_x_ave = float(re.split('[: ]', result_peskew_ave[2])[1])
    ...
    def VOA_calibration_New(self):
        ...
        #2022-10-09: add VOA flag read to skip the voa calibration test
        #read the flag here and skip the test
        mawin.CtrlB.write(b'fs400_eep_arg_read 2\r\n')
        voa_read=mawin.CtrlB.read_until(b'temperature').decode('utf-8')
        voa_flag = [i.split(':')[1][0] for i in voa_read.split('\n') if 'voa_choice' in i][0]
        if voa_flag=='0':
            print('VOA flag等于0，将不执行VOA校准程序')
        elif voa_flag=='1':
            print('VOA flag等于1，将执行VOA校准程序')
        ...
        if voa_flag=='1':
            ...
            *****Start the whole test steps*****
            ...
            for i in range(len(mawin.channel)):
                ...
                time.sleep(0.1)
                mawin.CtrlB.write(b'itla_read 0 0x30\n\r')
                re_wl = mawin.CtrlB.read_until(b'shell').decode('utf-8') #Consider whether to add this to all the byte to str,\\***errors='ignore'***\\)
                print(re_wl)
                while not 'read_itla' in re_wl:
                    time.sleep(1)
                    mawin.CtrlB.write(b'itla_read 0 0x30\n\r')
            ...
            worksht = wb.sheets(1)
            worksht.activate()
            worksht.range((1, 2)).value = sn
            worksht.range((2, 2)).value = mawin.desk
            worksht.range((3, 2)).value = mawin.temp[0]
            worksht.range((4, 2)).value = timestamp.split('_')[0]
            worksht.range((5, 2)).value = timestamp.split('_')[1]
            # write CLPD,FPGA,MCU information here into the test report
            # testBrdClp,ctlBrdClp,ctlBrdModVer,ctlBrdFPGAver,MCUver
            # worksht.range((2, 4)).value = testBrdClp
            # worksht.range((3, 4)).value = ctlBrdClp
            ...
            # worksht.range((6, 4)).value = MCUver
            worksht.range((1, 4)).value = mawin.sw
            worksht.range((1, 5)).value = 'VOA flag=0,Skip voa calibration!'
