...
def set_JINLIwave(obj,ch):
 ...
 'A2FF'] #from ch9 to ch72
    if not 8<int(ch)<73:
        ...
        return False
    obj.JinLi.timeout=60
    obj.JinLi.read_all()
    time.sleep(0.1)
    wl_set=cha[int(ch)-9]
    cmd='msa_write 0xb400 0x{}\r\n'.format(wl_set).encode('utf-8')
    obj.JinLi.write(cmd)
    print(obj.JinLi.read_until(b'ITTRA_Driver target'))
    print(obj.JinLi.read_all())
    cmd = b'msa_read 0xb400\r\n'
    obj.JinLi.write(cmd)
    time.sleep(0.3)
    s=obj.JinLi.read_all().decode('utf-8')
    print(s)
    while not '0xB400:' in s:
        obj.JinLi.write(cmd)
        time.sleep(0.3)
        s = obj.JinLi.read_all().decode('utf-8')
        print(s)
    s1=[i for i in s.split('\n') if '0xB400:' in i][0]
    wl_read=s1.split(':')[1].strip()[2:6]
    count=0
    while not wl_read==wl_set:
        count+=1
        print('Jinli Wavelength not corrent, reset for the {} time...'.format(count))
        cmd = 'msa_write 0xb400 0x{}\r\n'.format(wl_set).encode('utf-8')
        obj.JinLi.write(cmd)
        print(obj.JinLi.read_until(b'ITTRA_Driver target'))
        print(obj.JinLi.read_all())
        ...
        wl_read = s1.split(':')[1].strip()[2:6]
        if count==10:
            print('Cannot set to the correct wavelength, please check!!!')
            return False
    obj.JinLi.timeout = 3
    return True
